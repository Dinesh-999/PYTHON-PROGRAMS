# generate random wikipedia article , opens in web browser
from tkinter import *
...
generate_article = Button(text="GENERATE NEW ARTICLE", font=("Verdana", 20), pady=0, command=lambda: get_article(link),
                          borderwidth=10, bg="white")
generate_article.pack(side=BOTTOM)
title = Label(text="Wikipedia Article Generator", font=("Verdana", 20), justify=CENTER, bg="white")
title.pack(side=TOP)
link = Label(text="\nClick GENERATE to get a link to a random Wikipedia article", font=("Verdana", 20), justify=CENTER,
             wraplength=400, bg="white")
link.pack(side=TOP)
window.mainloop()
